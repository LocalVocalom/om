...
@Service
public class AppLauchServiceImpl implements AppLaunchService {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(AppLaunchService.class);

	@Override
	public AppLaunchDto appLaunch(long userId, int languageId, String appVersion, boolean langChange) {
		...
		String stateName = currentUser.getState();
		String otp = currentUser.getOtp();
		Integer otpVerify = currentUser.getOtpVerify();
		...
		try {
			AppPopupDto appPopup = appPopupService.getPrioritizedPopupDto(userId, appVersion, languageId, currentUser.getCreatedTime());
			appLaunchDto.setApp_popup(appPopup);
		} catch (Exception ex) {
			LOGGER.warn("some exception occured while adding app popup, exception={}", ex.getMessage());
			//appLaunchDto.setApp_popup(null);
		}
		...
		// Allowed for beta testers only 
		/*
		String betaUsers = dbConfigService.getProperty(Properties.BETA_USERS.getDefaultValue(), Properties.BETA_USERS.getDefaultValue());
		if(betaUsers.contains(String.valueOf(userId))) {
